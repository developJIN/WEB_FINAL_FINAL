/* assign each ID from html to each variables*/
...
/* check postal code using regular expression*/
function check(postalCode) {
  const regexpress = /^[ABCEGHJKLMNPRSTVXY]\d[A-Z]\s{0,1}\d[A-Z]\d$/;
  let val = false;
  if (regexpress.test(postalCode)) val = postalCode;
  return val;
}

/*function for submitting all data that user entered*/
form.onsubmit = function () {
  ...
  /* display error message for each inappropriate data*/
  if (
    !(
      first_name.value &&
      last_name.value &&
